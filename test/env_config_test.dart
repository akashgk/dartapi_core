...
void main() {
  group('EnvConfig defaults', () {
    ...
    test('returns string default', () => expect(config.host, equals('localhost')));
  ...
  group('EnvConfig reads values', () {
    test('reads string', () => expect(_cfg({'HOST': 'db.internal'}).host, equals('db.internal')));
    test('reads int', () => expect(_cfg({'PORT': '9090'}).port, equals(9090)));
    test('reads double', () => expect(_cfg({'RATIO': '3.14'}).ratio, equals(3.14)));
    test('reads bool true', () => expect(_cfg({'DEBUG': 'true'}).debug, isTrue));
    test('reads bool TRUE (case-insensitive)', () => expect(_cfg({'DEBUG': 'TRUE'}).debug, isTrue));
    test('reads bool non-true as false', () => expect(_cfg({'DEBUG': 'yes'}).debug, isFalse));
  });

  group('EnvConfig errors', () {
    test('throws MissingEnvException for required absent var', () {
      expect(() => _TestConfig(environment: {}).secret, throwsA(isA<MissingEnvException>()));
    });
    ...
    test('throws InvalidEnvException for unparseable int', () {
      expect(() => _cfg({'PORT': 'bad'}).port, throwsA(isA<InvalidEnvException>()));
    });

    test('throws InvalidEnvException for unparseable double', () {
      expect(() => _cfg({'RATIO': 'bad'}).ratio, throwsA(isA<InvalidEnvException>()));
    });
