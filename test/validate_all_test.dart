...
void main() {
  group('Map.validateAll', () {
    ...
    test('missing required field is collected as an error', () {
      ...
      ValidationException? caught;
      try {
        json.validateAll({'name': () => json.verifyKey<String>('name')});
      } on ValidationException catch (e) {
        caught = e;
      }
      expect(caught, isNotNull);
  ...
  group('ApiRoute — ValidationException returns 422 with errors list', () {
    test(
      'handler returns 422 with errors array on ValidationException',
      () async {
          ...
          dtoParser: (json) {
            json.validateAll({
              ...
              'name':
                  () => json.verifyKey<String>(
                    'name',
                    validators: [NotEmptyValidator('Name is required')],
                  ),
            });
            return json;
          },
